...
SUPPORTED_SYMBOLS = list(SYMBOL_TO_COINGECKO_ID.keys())


async def get_crypto_price(symbol: str) -> float:
    """Fetch the current USD price for a single crypto asset."""
    cg_id = SYMBOL_TO_COINGECKO_ID.get(symbol.upper())
    if not cg_id:
        raise ValueError(f"Unsupported asset: {symbol}. Supported: {SUPPORTED_SYMBOLS}")
